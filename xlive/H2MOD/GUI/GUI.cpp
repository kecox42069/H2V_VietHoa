...
int WINAPI XLiveInitializeEx(XLIVE_INITIALIZE_INFO* pXii, DWORD dwVersion)
{
	...
	if (pXii->pD3D)
	{
		...
		snprintf(serverStatus, 256, "Trang thai: Dang doi login....");

		buildText = new char[256];
		snprintf(buildText, 256, "Project Cartographer Viet Hoa (v%s) - Thoi Gian Build: %s %s", DLL_VERSION_STR, CompileDate, CompileTime);
...
void InitalizeFont(std::wstring strFontName, std::wstring& strFontPath, int size, IDirect3DDevice9* pD3Ddev, bool OnOff)
{
	if (OnOff)
	{
		addDebugText("Them font: %s", strFontPath.c_str());
		if(AddFontResource(strFontPath.c_str()) > 0)
		{
			addDebugText("Font da duoc add thanh cong.");
		}
		else
		{
			addDebugText("Khong the add font.");
		}
	}
...
void initFontsIfRequired()
{
	...
	D3DXCreateFont(pDevice, 10, 0, FW_NORMAL, 1, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Verdana", &smallFont);
